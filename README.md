...
- **OBS is on another PC:** run `pocket-overlay` on the PC the radio is plugged into, switch on **LAN** at the top of its settings page (or start it with `--lan`), and use the URL it shows. Allow it if Windows asks.
